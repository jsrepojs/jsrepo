...
**jsrepo** v3 is a complete rewrite of the **jsrepo** CLI and we haven't been shy about breaking changes. 

If you're coming from **jsrepo** v2 and wanting to quickly get started with **jsrepo** v3 you can run the following command in your existing **jsrepo** project:
...
**jsrepo** v3 is now partially **shadcn** compatible. You can now add and update items from **shadcn** meaning **jsrepo** now has access to an entire ecosystem of **shadcn** registries.
...
Now I did say *partially* compatible because **jsrepo** doesn't yet support updating imports from **shadcn** items.

### Greatly improved MCP responses

Thanks to adopting the more manual approach for defining a registry it's now possible to add metadata to you registry items greatly improving the responses you will get when using the **jsrepo** MCP server.
